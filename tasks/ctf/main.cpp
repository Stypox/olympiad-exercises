...
INLINE ui lowerPowerOf2(ui value) {
	value |= (value >> 1);
	value |= (value >> 2);
	value |= (value >> 4);
	value |= (value >> 8);
...
INLINE ui getWinner(const ui nrPlayers) {
	return 2 * (nrPlayers - lowerPowerOf2(nrPlayers)) + 1;
}
...
int main() {
	...
	for (ui gameIndex = 0, nrPlayers; gameIndex < Q; ++gameIndex) {
		in >> nrPlayers;
		out << getWinner(nrPlayers);
	}
	return 0;
}
