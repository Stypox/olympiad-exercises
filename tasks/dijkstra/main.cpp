...
struct Node {
	i val = std::numeric_limits<i>::max();
...
int main() {
	...
	for(i m = 0; m != M; ++m) {
		...
		in >> from >> to >> weight;
		--from; --to;
		nodes[from].links.push_back({to, weight});
	}
	nodes[start].val = 0;

	std::deque<i> toProcess{start};
	while (!toProcess.empty()) {
		for(auto&& link : nodes[toProcess.front()].links) {
			i newVal = nodes[toProcess.front()].val + link.weight;
			if (newVal < nodes[link.to].val) {
				nodes[link.to].val = newVal;
				toProcess.push_back(link.to);
			}
		}
		toProcess.pop_front();
	}
